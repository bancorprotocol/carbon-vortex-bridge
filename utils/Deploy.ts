...
const { TENDERLY_NETWORK_NAME }: EnvOptions = process.env as any as EnvOptions;
...
enum NewInstanceName {
    ...
    VortexOpStackBridge = 'VortexOpStackBridge',
...
const DeployedNewContracts = {
    ...
    VortexOpStackBridge: deployed<VortexOpStackBridge>(InstanceName.VortexOpStackBridge),
...
export const save = async (deployment: Deployment) => {
    ...
    // publish the contract to a tenderly fork or testnet
    if (!skipVerification) {
...
export const getNetworkNameById = (networkId: number | undefined): string => {
    if (networkId === undefined) {
        return 'mainnet'; // Assuming 'mainnet' is the default network
    }
