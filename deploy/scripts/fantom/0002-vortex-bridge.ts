...
const func: DeployFunction = async ({ getNamedAccounts }: HardhatRuntimeEnvironment) => {
    ...
    // Deploy Vortex Fantom Bridge contract
    await deploy({
        name: InstanceName.VortexFantomBridge,
        from: deployer,
    ...
    const vortexFantomBridge = await DeployedContracts.VortexFantomBridge.deployed();

    // grant admin permission to the bridge so it can withdraw funds
    const adminSigner = await ethers.getSigner(deployer);
    const vortexInstance = await ethers.getContractAt('VortexFantomBridge', vortex);
    await vortexInstance.connect(adminSigner).grantRole(Roles.Upgradeable.ROLE_ADMIN, vortexFantomBridge.address);
