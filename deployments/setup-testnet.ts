...
const fundAccount = async (account: string, fundingRequests: FundingRequest[]) => {
    ...
    for (const fundingRequest of fundingRequests) {
        ...
        const tokenContract = await Contracts.TestERC20Token.attach(fundingRequest.token);
        ...
        if (whaleBalance.lt(fundingRequest.amount)) {
            Logger.log(`Whale ${whale.address} has insufficient balance for ${fundingRequest.tokenName}`);
            continue;
        }
